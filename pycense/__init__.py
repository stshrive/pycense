...
def get_ignored_packages(packages):
    ignored  = [package for package in packages if not installed(package)]
    ignored += [package for package in installs if package not in packages]
    return  ignored
...
def main(path, output, exclude_requirements, exclude_imports):
    ...
    ignored = get_ignored_packages(packages)
